  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-6">
      <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest text-quiz-gold">
        <span>{room.isBoss ? '👹 Boss Room' : `Room ${room.index + 1}/${dungeonLength}`}</span>
        <span className="text-quiz-ink/70">⏱ {secondsLeft}s</span>
      </div>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-wide text-quiz-ink/70">
            <span>Your HP</span>
            <span>{myHp}/{STARTING_HP}</span>
          </div>
          <div className="mt-1 h-2 w-full rounded-full bg-quiz-hpTrack">
            <div className="h-2 rounded-full bg-quiz-hp transition-all" style={{ width: `${Math.round((myHp / STARTING_HP) * 100)}%` }} />
          </div>
        </div>
